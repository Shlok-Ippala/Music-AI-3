...
Extracted from TwelveTake REAPER MCP Server.
Provides all REAPER DAW control functions via file-based or HTTP bridge communication.

Original author: TwelveTake Studios LLC
License: MIT
"""
...
def _default_bridge_dir():
    """Get the default bridge directory based on the current platform."""
    system = platform.system()
    if system == "Windows":
        base = os.environ.get("APPDATA", os.path.expanduser("~"))
        return Path(base) / "REAPER" / "Scripts" / "mcp_bridge_data"
    elif system == "Darwin":
        return Path.home() / "Library" / "Application Support" / "REAPER" / "Scripts" / "mcp_bridge_data"
    else:
        return Path.home() / ".config" / "REAPER" / "Scripts" / "mcp_bridge_data"
...
def tool(func):
    """Register a function as a REAPER tool."""
    TOOLS[func.__name__] = func
    return func


# Request counter for file-based communication
request_counter = 0
...
async def add_midi_note(
    ...
    Args:
        ...
        start_position: Start position in seconds relative to item start.
    ...
    Returns:
        Object with success status.
    """
    return await reaper_call("InsertMIDINote", track_index, item_index, pitch, start_position, length, velocity, channel)
...
async def add_midi_notes_batch(
    ...
    Args:
        ...
        notes: List of note dicts with keys: pitch, velocity, start_position (seconds from item start), length (seconds), channel (optional).
